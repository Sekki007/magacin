-- POPRAVKA za postojeću bazu — pokreni ceo fajl u Supabase SQL Editor
-- Rešava: prazan dropdown (nedostaju tabele) i grešku pri brisanju
...
-- 2) Kolona boja na artiklima (ako fali)
alter table public.poklopci_artikli add column if not exists boja text;
update public.poklopci_artikli set boja = 'Nepoznata' where boja is null or boja = '';
update public.poklopci_artikli set model = naziv where model is null or model = '';

-- 3) RLS i politike za katalog
alter table public.poklopci_modeli enable row level security;
alter table public.poklopci_boje enable row level security;

drop policy if exists "poklopci_modeli_all" on public.poklopci_modeli;
drop policy if exists "poklopci_boje_all" on public.poklopci_boje;
...
-- 4) Popuni katalog (isti sadržaj kao poklopci_katalog.sql)
